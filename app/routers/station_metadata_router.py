...
from ..auth_utils import get_current_admin_user, get_current_active_user
from sqlmodel import select # Import select for queries
# Ensure all necessary models are imported
from ..core.models import (
    StationMetadata, StationMetadataCreate, StationMetadataRead,
    StationMetadataReadWithLogs, StationMetadataUpdate,
    OffloadLog, OffloadLogCreate, OffloadLogRead, User as UserModel
)
...
@router.post("/station_metadata/", response_model=models.StationMetadataRead, status_code=status.HTTP_201_CREATED, tags=["Station Metadata Admin"])
async def create_or_update_station_metadata_on_router(
    ...
    current_user: Annotated[UserModel, Depends(get_current_admin_user)] # Admin required to create/update
):
    logger.info(f"ROUTER: User '{current_user.username}' attempting to create/update station: {station_data.station_id}")
    
    existing_station = session.get(models.StationMetadata, station_data.station_id)
    
    if existing_station:
        logger.info(f"ROUTER: Station '{station_data.station_id}' already exists. Updating.")
...
@router.get("/station_metadata/{station_id}", response_model=models.StationMetadataReadWithLogs, tags=["Station Metadata"])
async def get_station_metadata_by_id_on_router(
    station_id: str,
    session: Annotated[SQLModelSession, Depends(get_db_session)],
    current_user: Annotated[UserModel, Depends(get_current_active_user)] # Any active user can view
):
    logger.info(f"ROUTER: User '{current_user.username}' fetching station metadata for ID: {station_id}")
...
@router.put("/station_metadata/{station_id}", response_model=models.StationMetadataRead, tags=["Station Offload Management"])
async def update_station_metadata_fields(
    station_id: str,
    station_update_data: models.StationMetadataUpdate,
    session: Annotated[SQLModelSession, Depends(get_db_session)],
    current_user: Annotated[UserModel, Depends(get_current_active_user)] # Any authenticated user can update
):
    logger.info(f"ROUTER: User '{current_user.username}' attempting to update station fields for: {station_id}")
    ...
    if not db_station_metadata:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Station metadata not found")
    ...
    logger.info(f"ROUTER: Station '{station_id}' fields updated by user '{current_user.username}'.")
...
@router.get("/station_metadata/", response_model=List[models.StationMetadataRead], tags=["Station Metadata"])
async def search_station_metadata_on_router(
    query: Optional[str] = Query(None, description="Search query for station_id (substring match)"),
    limit: int = Query(10, gt=0, le=100, description="Maximum number of stations to return"),
    session: SQLModelSession = Depends(get_db_session),
    current_user: UserModel = Depends(get_current_active_user)
):
    logger.info(f"ROUTER: User '{current_user.username}' searching station metadata with query: '{query}', limit: {limit}")
    ...
    if query:
        ...
        statement = statement.where(models.StationMetadata.station_id.ilike(f"%{query}%"))
    
    statement = statement.order_by(models.StationMetadata.station_id).limit(limit) # Added order_by for consistent results
...
@router.post("/station_metadata/{station_id}/offload_logs/", response_model=models.OffloadLogRead, status_code=status.HTTP_201_CREATED, tags=["Station Offload Management"])
async def create_offload_log_for_station(
    station_id: str,
    offload_log_in: models.OffloadLogCreate,
    session: Annotated[SQLModelSession, Depends(get_db_session)],
    current_user: Annotated[UserModel, Depends(get_current_active_user)] # Any authenticated user can log
):
    logger.info(f"ROUTER: User '{current_user.username}' logging offload for station: {station_id}")
    db_station_metadata = session.get(models.StationMetadata, station_id)
    if not db_station_metadata:
        logger.warning(f"ROUTER: Station metadata not found for ID '{station_id}' when logging offload.")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Station metadata not found for logging offload")
    ...
    if db_offload_log.offload_end_time_utc:
        db_station_metadata.last_offload_timestamp_utc = db_offload_log.offload_end_time_utc
    elif db_offload_log.log_timestamp_utc: # Fallback if end time not provided but log is made
        db_station_metadata.last_offload_timestamp_utc = db_offload_log.log_timestamp_utc
    ...
    logger.info(f"ROUTER: Offload logged for station '{station_id}' by user '{current_user.username}'. Log ID: {db_offload_log.id}")
    return db_offload_log

@router.get("/stations/status_overview", response_model=List[Dict[str, Any]], tags=["Station Status Overview"]) # Using Dict for augmented response
async def get_station_offload_status_overview(
    session: Annotated[SQLModelSession, Depends(get_db_session)],
    current_user: Annotated[UserModel, Depends(get_current_active_user)] # Any active user can view
):
    logger.info(f"User '{current_user.username}' requesting station offload status overview.")
    
    statement = select(models.StationMetadata).order_by(models.StationMetadata.station_id)
    ...
    for station in stations_metadata:
        ...
        if station.last_offload_timestamp_utc:
            ...
            if retrieved_ts.tzinfo is None or retrieved_ts.tzinfo.utcoffset(retrieved_ts) is None:
                retrieved_ts_aware = retrieved_ts.replace(tzinfo=timezone.utc)
            else:
                retrieved_ts_aware = retrieved_ts

            last_offload_timestamp_str = retrieved_ts_aware.strftime('%Y-%m-%d %H:%M:%S UTC')
        ...
        if station.offload_logs:
            ...
            sorted_logs = sorted(station.offload_logs, key=lambda log: log.log_timestamp_utc, reverse=True)
            if sorted_logs:
                ...
                latest_distance_command_sent_m_str = str(latest_log.distance_command_sent_m) if latest_log.distance_command_sent_m is not None else "---"
                latest_time_first_command_sent_utc_str = format_dt(latest_log.time_first_command_sent_utc)
                latest_offload_start_time_utc_str = format_dt(latest_log.offload_start_time_utc)
                latest_offload_end_time_utc_str = format_dt(latest_log.offload_end_time_utc) # This is also the basis for last_offload_timestamp_str if this log is the one that set it
                latest_departure_date_str = format_dt(latest_log.departure_date)
                latest_was_offloaded_str = "Yes" if latest_log.was_offloaded is True else ("No" if latest_log.was_offloaded is False else "---")
                latest_offload_notes_file_size_str = latest_log.offload_notes_file_size or "---"
        # New status logic
        ...
        overview_list.append({
            ...
            "station_settings": station.station_settings or "---", # Add station settings
            # "last_offload_by_glider": station.last_offload_by_glider, # No longer directly in table
            "last_offload_timestamp_str": last_offload_timestamp_str,
            "status_text": status_text,
            "status_color": status_color_key, # Send the key for JS to map to CSS class
            "display_status_override": station.display_status_override, # For modal pre-fill
