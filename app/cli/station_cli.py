...
def get_admin_token():
    ...
    """
    try:
        token_url = BASE_API_URL.replace('/api', '/token') # Token endpoint is at the app root
        console.print(f"Attempting to get admin token from: [cyan]{token_url}[/cyan] for user [yellow]{ADMIN_USERNAME}[/yellow]")
        response = httpx.post(
            token_url,
        ...
        console.print(f"[bold red]HTTP Error getting admin token: {e.response.status_code} - {e.response.text}[/bold red]")
...
def import_station_metadata(
    csv_file: Annotated[Path, typer.Option(help="Path to the CSV file containing station metadata.", exists=True, file_okay=True, dir_okay=False, readable=True)],
):
    """
    ...
    """
    console.print(f"Attempting to import station metadata from: [cyan]{csv_file}[/cyan]")

    try:
        df = pd.read_csv(csv_file, dtype=str) # Read all as string initially to preserve formatting like leading zeros
    except Exception as e:
        console.print(f"[bold red]Error reading CSV file '{csv_file}': {e}[/bold red]")
        raise typer.Exit(code=1)

    expected_headers = [
        "station_id", "serial_number", "modem_address",
        "bottom_depth_m", "waypoint_number", "last_offload_by_glider",
        "station_settings", "notes"
    ]

    if "station_id" not in df.columns:
        console.print(f"[bold red]CSV file must contain a 'station_id' column.[/bold red]")
        raise typer.Exit(code=1)

    admin_token = get_admin_token()
    if not admin_token:
        console.print("[bold red]Could not authenticate as admin. Aborting import.[/bold red]")
        raise typer.Exit(code=1)

    api_headers = {"Authorization": f"Bearer {admin_token}", "Content-Type": "application/json"}
    success_count = 0
    fail_count = 0
    skipped_count = 0

    # Use a session for multiple requests
    with httpx.Client(base_url=BASE_API_URL, headers=api_headers, timeout=20.0) as client:
        for index, row in df.iterrows():
            station_id = row.get("station_id")
            if pd.isna(station_id) or not str(station_id).strip():
                console.print(f"[yellow]Skipping row {index+2} due to missing or empty station_id.[/yellow]")
                skipped_count += 1
                continue
                ...
                if header in row and pd.notna(row[header]):
                    ...
                    if header == "modem_address":
                        try:
                            ...
                            console.print(f"[yellow]Warning for station '{station_id}': Invalid modem_address '{value}'. Skipping field.[/yellow]")
                    elif header == "bottom_depth_m":
                        try:
                            payload[header] = float(value)
                        except ValueError:
                            console.print(f"[yellow]Warning for station '{station_id}': Invalid bottom_depth_m '{value}'. Skipping field.[/yellow]")
                    else:
                        payload[header] = value
            
            # Correctly construct the URL for logging and ensure the post path is relative
            target_path = "station_metadata/" 
            full_post_url = f"{str(client.base_url).rstrip('/')}/{target_path.lstrip('/')}"
            console.print(f"Processing station: [blue]{station_id}[/blue], Payload: {payload}")
            ...
            try:
                ...
                if response.status_code == 201: # Created
                    console.print(f"  [green]Successfully CREATED station: {station_id}[/green]")
                    success_count += 1
                elif response.status_code == 200: # OK (likely updated if your POST also handles updates)
                    console.print(f"  [green]Successfully UPDATED station: {station_id}[/green]")
                    success_count += 1
                else:
                    ...
                    console.print(f"  [red]Failed for station '{station_id}': {response.status_code} - {error_detail}[/red]")
                    fail_count += 1
            except httpx.RequestError as e_req:
                console.print(f"  [red]Request error for station '{station_id}': {e_req}[/red]")
                fail_count += 1
            except Exception as e_gen:
                console.print(f"  [red]Generic error processing station '{station_id}': {e_gen}[/red]")
                fail_count += 1

    console.print(f"\n[bold blue]Import complete.[/bold blue]")
    console.print(f"  Successfully processed (created/updated): [green]{success_count}[/green]")
