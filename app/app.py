...
def load_data_source(report_type: str, mission_id: str):
    ...
    try:
        ...
        # Pass base_path to mission_core.load_report
        return mission_core.load_report(report_type, mission_id, base_path=settings.local_data_base_path)
    ...
    try:
        ...
        # Pass base_url to mission_core.load_report
        return mission_core.load_report(report_type, mission_id, base_url=settings.remote_data_url)
...
async def home(request: Request, mission: str = "m203", hours: int = 72):
    ...
    power_status = mission_core.get_power_status(df_power) if df_power is not None and not df_power.empty else None
    ctd_status = mission_core.get_ctd_status(df_ctd) if df_ctd is not None and not df_ctd.empty else None
    weather_status = mission_core.get_weather_status(df_weather) if df_weather is not None and not df_weather.empty else None
    wave_status = mission_core.get_wave_status(df_waves) if df_waves is not None and not df_waves.empty else None
    ais_summary = mission_core.get_ais_summary(df_ais, max_age_hours=hours) if df_ais is not None else [] # get_ais_summary handles empty df
    recent_errors_list = mission_core.get_recent_errors(df_errors, max_age_hours=hours)[:20] if df_errors is not None else [] # get_recent_errors handles empty df
    ...
    return templates.TemplateResponse("index.html", {
        ...
        "ais": ais_summary if ais_summary is not None else "Data unavailable", # ais_summary can be an empty list
