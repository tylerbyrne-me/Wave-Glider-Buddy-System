...
MARINE_API_BASE_URL = "https://marine-api.open-meteo.com/v1/marine" # Specialized marine data
GENERAL_API_BASE_URL = "https://api.open-meteo.com/v1/forecast"

# Parameters for the Marine API - focused on marine data
MARINE_API_HOURLY_PARAMS = "wave_height,wave_direction,wave_period,ocean_current_velocity,ocean_current_direction"
...
async def get_general_meteo_forecast(lat: float, lon: float) -> Optional[Dict[str, Any]]:
    """
    Fetches general weather forecast data (temperature, weathercode, precipitation, wind).
    """
    final_data: Optional[Dict[str, Any]] = None
    base_api_params = {"latitude": lat, "longitude": lon, "timezone": "GMT"}

    logger.info(f"Attempting to fetch general forecast from: {GENERAL_API_BASE_URL} for lat={lat}, lon={lon}.")
    try:
        general_api_params = {**base_api_params, "hourly": GENERAL_API_HOURLY_PARAMS}
        final_data = await _fetch_forecast_data(GENERAL_API_BASE_URL, general_api_params)
        if final_data:
            ...
            return None
    except httpx.HTTPStatusError as e_gen_http:
        logger.error(f"General forecast API failed with HTTP error: {e_gen_http.response.status_code} - {e_gen_http}")
        return None
    except Exception as e_gen_other:
...
async def get_marine_meteo_forecast(lat: float, lon: float) -> Optional[Dict[str, Any]]:
    ...
    logger.info(f"Attempting to fetch marine forecast from: {MARINE_API_BASE_URL} for lat={lat}, lon={lon}")
    try:
        ...
        if final_data:
            ...
            return None
    except httpx.HTTPStatusError as e_marine_http:
        logger.error(f"Marine forecast API failed with HTTP error: {e_marine_http.response.status_code} - {e_marine_http}")
        return None
    except Exception as e_marine_other:
